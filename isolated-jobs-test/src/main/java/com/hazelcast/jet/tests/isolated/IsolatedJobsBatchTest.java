 ...
 * To run this test locally, you have to use EE version (it can be forced by adding dependency to hazelcast-enterprise in the pom.xml)
 * and license must be set (it can be done in {@link AbstractSoakTest#run(String[])})
 */
public class IsolatedJobsBatchTest extends AbstractSoakTest {
    ...
    protected void init(HazelcastInstance client) throws Exception {
        sleepBetweenNewJobsInMinutes = propertyInt("sleepBetweenNewJobsInMinutes", DEFAULT_SLEEP_BETWEEN_NEW_JOBS_IN_MINUTES);
    }
